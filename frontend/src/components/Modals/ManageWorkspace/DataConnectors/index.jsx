...
export const DATA_CONNECTORS = (t) => ({
  github: {
    name: t("dataConnectors.github.name"),
    image: ConnectorImages.github,
    description: t("dataConnectors.github.description"),
    options: <GithubOptions />,
  },
  gitlab: {
    name: t("dataConnectors.gitlab.name"),
    image: ConnectorImages.gitlab,
    description: t("dataConnectors.gitlab.description"),
    options: <GitlabOptions />,
  },
  "youtube-transcript": {
    name: t("dataConnectors.youtubeTranscript.name"),
    image: ConnectorImages.youtube,
    description: t("dataConnectors.youtubeTranscript.description"),
    options: <YoutubeOptions />,
  },
  "website-depth": {
    name: t("dataConnectors.websiteDepth.name"),
    image: ConnectorImages.websiteDepth,
    description: t("dataConnectors.websiteDepth.description"),
    options: <WebsiteDepthOptions />,
  },
  confluence: {
    name: t("dataConnectors.confluence.name"),
    image: ConnectorImages.confluence,
    description: t("dataConnectors.confluence.description"),
...
export default function DataConnectors() {
  ...
  const connectors = DATA_CONNECTORS(t);
  const filteredConnectors = Object.keys(connectors).filter((slug) =>
    connectors[slug].name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="flex upload-modal -mt-10 relative min-h-[80vh] w-[70vw]">
          ...
          {filteredConnectors.length > 0 ? (
            filteredConnectors.map((slug, index) => (
              <ConnectorOption
                ...
                image={connectors[slug].image}
                name={connectors[slug].name}
                description={connectors[slug].description}
              />
            ))
          ) : (
            <div className="text-white text-center mt-4">
              {t("dataConnectors.noResults")}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="w-full p-4 top-0 text-white min-w-[500px]">
        {connectors[selectedConnector].options}
      </div>
    </div>
  );
}
