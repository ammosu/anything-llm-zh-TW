...
const ManageWorkspace = ({ hideModal = noop, providedSlug = null }) => {
          ...
          <div className="relative p-6 border-b rounded-t border-theme-modal-border">
            <div className="w-full flex gap-x-2 items-center">
              <h3 className="text-xl font-semibold text-white overflow-hidden overflow-ellipsis whitespace-nowrap">
                {t('manageWorkspace.editing', { name: workspace.name })}
              </h3>
            </div>
            <button
              onClick={hideModal}
          ...
          >
            <div className="py-7 px-9 space-y-2 flex-col">
              <p className="text-white">
                {t('manageWorkspace.mobileWarning')}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="w-screen h-screen fixed top-0 left-0 flex justify-center items-center z-modal">
      <div className="backdrop h-full w-full absolute top-0 z-10" />
      <div className="absolute max-h-full w-fit transition duration-300 z-modal md:overflow-y-auto py-10">
  ...
  return (
    <div className="w-full flex justify-center z-10 relative">
      <div className="gap-x-2 flex justify-center -mt-[68px] mb-10 bg-theme-bg-secondary p-1 rounded-xl shadow border-2 border-theme-modal-border w-fit">
        ...
        >
          {t('manageWorkspace.dataConnectors')}
        </button>
      </div>
    </div>
  );
