...
export default function EditPresetModal({
  ...
  const handleDelete = async () => {
    if (!window.confirm(t("editPreset.confirmDelete"))) return;
        ...
        >
          <form onSubmit={handleSubmit}>
              ...
              <div className="w-full flex flex-col gap-y-4">
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      type="text"
                      name="command"
                      placeholder={t("editPreset.commandPlaceholder")}
                      value={command}
                ...
                <div>
                  <label
                    ...
                    className="block mb-2 text-sm font-medium text-white"
                  >
                    {t("editPreset.promptLabel")}
                  </label>
                  <textarea
                    name="prompt"
                    placeholder={t("editPreset.promptPlaceholder")}
                ...
                <div>
                  <label
                    ...
                    className="block mb-2 text-sm font-medium text-white"
                  >
                    {t("editPreset.descriptionLabel")}
                  </label>
                  <input
                    type="text"
                    name="description"
                    defaultValue={preset.description}
                    placeholder={t("editPreset.descriptionPlaceholder")}
                    required={true}
                    className="border-none bg-theme-settings-input-bg w-full text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
                  />
                </div>
            ...
            <div className="flex w-full justify-between items-center p-6 space-x-2 border-t border-theme-modal-border rounded-b">
              ...
              >
                {deleting
                  ? t("editPreset.deleting")
                  : t("editPreset.deleteButton")}
              </button>
              <div className="flex space-x-2">
                ...
                >
                  {t("editPreset.cancelButton")}
                </button>
                <button
                  type="submit"
                  className="transition-all duration-300 bg-white text-black hover:opacity-60 px-4 py-2 rounded-lg text-sm"
                >
                  {t("editPreset.saveButton")}
                </button>
              </div>
            </div>
          </form>
