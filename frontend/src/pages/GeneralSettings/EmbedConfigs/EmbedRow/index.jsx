...
export default function EmbedRow({ embed }) {
  ...
  const handleSuspend = async () => {
    if (
      !window.confirm(
        t("embedRow.confirmDisable", {
          embedName: embed.workspace.name,
        })
      )
    )
      return false;
    ...
    if (success) {
      showToast(
        t(enabled ? "embedRow.disabledSuccess" : "embedRow.enabledSuccess"),
        "success",
        { clear: true }
      );
      setEnabled(!enabled);
  ...
  const handleDelete = async () => {
    ...
    if (success) {
      ...
      showToast(t("embedRow.deleteSuccess"), "success", { clear: true });
    }
  };
      ...
      >
        ...
        <td className="px-6 py-4 flex items-center gap-x-6">
          ...
          <>
            ...
            >
              {enabled ? t("embedRow.disable") : t("embedRow.enable")}
            </button>
            <button
...
function ActiveDomains({ domainList }) {
  const { t } = useTranslation();
  if (!domainList) return <p>{t("embedRow.allDomains")}</p>;
  try {
    ...
    return <p>{t("embedRow.allDomains")}</p>;
  }
}
